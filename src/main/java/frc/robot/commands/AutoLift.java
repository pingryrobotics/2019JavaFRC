...
public class AutoLift extends CommandGroup {
  ...
  public AutoLift() {
    ...
    addParallel(new DriveAtPower(0.3, 10)); //Drives forward until retracted
    ...
    addParallel(new DriveAtPower(0.4, 10));  //Drive forward to get on the platform
    addSequential(new ExtendBack(-0.75), 3);

    //Finish getting on the platform
    addParallel(new DriveAtPower(0.1, 2));
  }
}
